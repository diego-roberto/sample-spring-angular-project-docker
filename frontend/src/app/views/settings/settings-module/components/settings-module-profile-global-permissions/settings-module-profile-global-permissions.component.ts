...
@Component({
  selector: 'settings-module-profile-global-permissions',
  templateUrl: './settings-module-profile-global-permissions.component.html',
  styleUrls: ['./settings-module-profile-global-permissions.component.scss']
})
export class SettingsModuleProfileGlobalPermissionsComponent implements OnInit {
  ...
  totalSteps = 1;
  @Input() title = 'Gerenciar as Permissões do perfil ';
  step = 1;
  workerQualificationsItem: QualificationsItem;
  ...
  ngOnInit() {
    ...
    this.permissionService.getAllPermissions().flatMap(
      permissions => {
        ...
        return this.userProfileService.getPermissionsByUserProfile(this.profileId);
      }
    ).subscribe(
  ...
  save() {
    ...
    this.userProfileService.addPermissionsToUserProfile(this.profileId, permissionIds).subscribe(result => {
