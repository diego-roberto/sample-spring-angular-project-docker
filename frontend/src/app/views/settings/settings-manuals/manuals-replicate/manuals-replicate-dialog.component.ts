...
@Component({
  selector: 'manuals-replicate-dialog',
  templateUrl: './manuals-replicate-dialog.component.html',
  styleUrls: ['./manuals-replicate-dialog.component.scss']
})

export class ManualsReplicateDialogComponent implements OnInit {
  ...
  title: string = "Replicar Manual nas Empresas";
  loading: boolean = false;

  constructor(public dialogRef: MdDialogRef<ManualsReplicateDialogComponent>,
  ...
  replicate(): any {
    ...
    this.filesService.replicate(this.fileId, selectedItems).subscribe(item => {
      this.appMessage.showSuccess("Manuais replicados com sucesso.");
    })
