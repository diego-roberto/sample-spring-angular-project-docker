...
@Component({
  selector: 'worker-qualification-chart',
  templateUrl: './worker-qualification-chart.component.html',
  styleUrls: ['./worker-qualification-chart.component.scss']
})
export class WorkerQualificationChartComponent implements OnInit {

  emptyChart = true;
  chartTitle = 'Habilitações';
  chartOption;
  filterRequest;
  first = true;
  ...
  findValues() {
    this.qualificationsService.getWorkerQualificationChart(this.filterRequest).subscribe(items => {
  ...
  showInfo() {
    const listDescription = [
      'HABILITAÇÕES: mostra o total de habilitações por status (atrasado ou realizado) no período e nas obras selecionadas vinculadas aos trabalhadores próprios e terceiros.'
    ];
  ...
  getItem(items: WorkerQualificationChart[], legend: string): WorkerQualificationChart {
    let temp = null;
    items.forEach(item => {
  ...
  buildChart(items: WorkerQualificationChart[]) {
